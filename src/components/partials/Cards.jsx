    ...
    <div className='flex flex-wrap w-full h-full px-[5%] bg-[#1F1E24] '>
        ...
        <Link to={`/${c.media_type || title}/details/${c.id}`} className='relative w-[25vh] mr-[5%] mb-[5%]' key={i}>
          <img className='shadow-[8px_17px_38px_2px_rgba(0,0,0,.5)] h-[40vh] object-cover ' src={`https://image.tmdb.org/t/p/original/${ c.poster_path || c.backdrop_path || c.profile_path }`} alt="" />
           ...
           {c.vote_average && (
            <div className='absolute right-[-10%] bottom-[20%] rounded-full bg-yellow-600 text-xl text-white font-semibold w-[6vh] h-[6vh] flex justify-center items-center '>{(c.vote_average * 10).toFixed()}<sup>%</sup></div>)}
